...
module.exports = function(jobs) {
  ...
  var SLAChangeProccess = function(job, done){
    ...
    Solicitude.findById(job.data.solicitudeId, function(err, solicitude) {
      if (!err) {
        ...
        if (solicitude.state.type === 'PROCCESS') {
  ...
  jobs.process('CreateSLA', 10, SLAProccess);
  jobs.process('ChangeSLA', 10, SLAChangeProccess);
  jobs.process('RemoveSLA', 10, SLARemoveProccess);
};
