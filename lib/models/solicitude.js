  ...
  for (var i = 0; i < that.involved.length; i++) {
      ...
      User.findById(that.involved[i].user, function (err, user) {
        if (!err) {
          ...
          if (typeof(conf[user.role]) !== 'undefined') {

            console.log('this.involved[i]:', user.role);
            console.log('conf[user.role]:', conf[user.role]);

            if (conf[user.role].action.indexOf('notification') > -1) {

              jobs.create('CreateNotification', {
                to: user._id,
                from: that.state.type === 'QUEUE_VALIDATION' ? that.createdBy : that.updatedBy,
                message: conf[user.role]['notification'],
            ...
            if (conf[user.role].action.indexOf('email') > -1) {

              var _data = {};
              if (conf[user.role].email.data.indexOf('name') > -1) _data['name'] = user.userInfo.name;
              if (conf[user.role].email.data.indexOf('code') > -1) _data['code'] = that.code;
              if (conf[user.role].email.data.indexOf('url') > -1) _data['url'] = global.host+'/solicitude/'+that._id;
              if (conf[user.role].email.data.indexOf('comment') > -1) _data['comment'] = that.message[that.state.type].content;
              ...
              jobs.create('sendEmail', {
                ...
                subject: conf[user.role].subject,
                content: {
                  data: _data,
                  type: user.role.toLowerCase()+'_'+that.state[user.role].toLowerCase()
                }
              })
              .priority('high')
              .save()
