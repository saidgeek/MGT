...
exports.update = function(req, res){
  ...
  Solicitude.findById(req.params.id, function (err, solicitude) {
    ...
    if (~['QUEUE_VALIDATION', 'ASSIGNED_TO_MANAGER', 'ASSIGNED_TO_PROVIDER'].indexOf(solicitude.state.type) 
      && ~['CANCEL', 'REJECTED_BY_MANAGER', 'REJECTED_BY_CLIENT'].indexOf(req.body.solicitude.nextState) === -1) {
      User.findById(_userId, function(err, user){
        ...
        solicitude.save(function (err) {
          console.log('solicitude save 1 err', err);
          if (err) return res.json(400, err);
          return res.json(200, solicitude);
        });
      });
      ...
      solicitude.save(function (err) {
        console.log('solicitude save 2 err', err);
        if (err) return res.json(400, err);
        return res.json(200, solicitude);
      });
    };
