...
exports.list = function(req, res) {
  ...
  if (_involved !== '' && _involved !== null) {
    query = {
      state: { $in: _state },
      'involved.user': _involved
    };
  }

  Solicitude.find(query)
  .populate('createdBy')
...
exports.groups = function (req, res) {
  ...
  Solicitude.aggregate([{ $match: { state: { $in: req.query.states }  } }, { $group: { _id: '$state', count: { $sum: 1 } } }], function (err, states) {
    if (err) return res.json(400, err);
    if (!states) return res.json(404);
    _states = states;
    Solicitude.aggregate([{ $match: { state: { $in: req.query.states }  } }, { $group: { _id: '$priority', count: { $sum: 1 } } }], function (err, priorities) {
      if (err) return res.json(400, err);
...
exports.update = function(req, res){
  ...
  Solicitude.findById(req.params.id, function (err, solicitude) {
    ...
    if (typeof(req.body.solicitude.responsible) !== 'undefined') solicitude.responsible = req.body.solicitude.responsible;
    if (typeof(req.body.solicitude.category) !== 'undefined') solicitude.ticket.category = req.body.solicitude.ticket.category;
    ...
    if (typeof(req.body.solicitude.provider) !== 'undefined') solicitude.provider = req.body.solicitude.provider;
    ...
    // AGREGANDOO LOS USUARIOS INBOLUCRADOS
