...
exports.search = function(req, res) {
  ...
  Solicitude.search({ query: req.params.q.replace(/\+/g, ' ') }, function(err, solicitudes) {
    if (err) return res.json(400, err);

    return res.json(200, solicitudes.hits);
  });
...
exports.update = function(req, res){
  ...
  Solicitude.findById(req.params.id, function (err, solicitude) {
    ...
    if (typeof(solicitude.priority) === 'undefined' || solicitude.priority === null) solicitude.priority = req.body.solicitude.priority || null;
    if (solicitude.responsible === null) solicitude.responsible = req.body.solicitude.responsible || null;
    if (solicitude.ticket.category === null) solicitude.ticket.category = req.body.solicitude.ticket.category || null;
    ...
    if (typeof(req.body.solicitude.message) !== 'undefined') {
      ...
      if (solicitude.state.type === 'CANCELED') {
        to = solicitude.applicant;
      };
      if (['PAUSED', 'REJECTED_BY_MANAGER', 'ASSIGNED_TO_PROVIDER'].indexOf(solicitude.state.type) > -1) {
        to = solicitude.provider;
      };
      if (['REJECTED_BY_CLIENT', 'ASSIGNED_TO_MANAGER', 'PROCCESS'].indexOf(solicitude.state.type) > -1) {
        to = solicitude.responsible;
      };

      Comment.add('solicitude.'+solicitude.state.type.toLowerCase(), to, req.params.id, req.body.solicitude.message, [], solicitude.updatedBy);
    };
