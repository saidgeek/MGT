...
exports.list = function(req, res) {
  ...
  query += ", \"archivedAt."+req.user.role+"\": null ";
  query += ", \"deleted\": false ";
  ...
  .exec(function(err, solicitudes){
    ...
    if (!solicitudes) return res.json(404);
      ...
      function(err, results){
        if (err) return res.json(400, err);
        return res.json(200, { solicitudes: results[0], totalItems: count });
      });
...
exports.search = function(req, res) {
  ...
  Solicitude.searching(req.params.q, req.user.userInfo, function(err, solicitudes) {
    if (err) return res.json(400, err);
    return res.json(200, {solicitudes: solicitudes});
  });
...
exports.list_archived = function(req, res) {
  ...
  query += "\"archivedAt."+req.user.role+"\": { \"$ne\": null } ";
  query += ", \"deleted\": false ";
...
exports.show = function(req, res){
  ...
  .exec(function(err, solicitude){
    ...
    var now = moment(Date.now());
    var start = moment(solicitude.startedAt);
    var end = moment(solicitude.endedAt);

    var diff = end.diff(now, 'milliseconds');

    solicitude.duration = diff;
...
exports.update = function(req, res){
  ...
  Solicitude.findById(req.params.id, function (err, solicitude) {
    ...
    if (typeof(req.body.solicitude.message) !== 'undefined') {
      ...
      if (['PAUSED', 'REJECTED_BY_MANAGER', 'ASSIGNED_TO_PROVIDER', 'PROCCESS'].indexOf(solicitude.state.type) > -1) {
        Comment.add('Solicitude.provider', solicitude.provider, req.params.id, req.body.solicitude.message, [], solicitude.updatedBy);
      };
      if (['REJECTED_BY_CLIENT', 'ASSIGNED_TO_MANAGER'].indexOf(solicitude.state.type) > -1) {
        Comment.add('Solicitude.internal', solicitude.responsible, req.params.id, req.body.solicitude.message, [], solicitude.updatedBy);
      };
    };
...
exports.archived = function (req, res) {
  ...
  Solicitude.archived(req.params.id, req.user.role, req.user.id, function (err) {
    if (err) return res.json(400, err);
    return res.json(200);
  });
...
exports.delete = function(req, res) {
  if (!req.params.id) return res.json(403);
  Solicitude.delete(req.params.id, req.user.id, function(err) {
    if (err) return res.json(400, err);
    return res.json(200);
  });
